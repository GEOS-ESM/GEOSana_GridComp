...
!     i_pblsld  - index in awork array holding pblh info
!     i_pblgld  - index in awork array holding pblh info
!     i_pblrd   - index in awork array holding pblh info
...
!$$$
  ...
  use obsmod, only: iout_sst,iout_pw,iout_t,iout_rw,iout_dw,&
       ...
       iout_gust,iout_vis,iout_pblri,iout_pblrf,iout_pblsld,iout_pblgld,iout_pblrd,&
       iout_wspd10m,iout_td2m,& 
       ...
       mype_vis,mype_pblri,mype_pblrf,mype_pblsld,mype_pblgld,mype_pblrd,&
       mype_wspd10m,mype_td2m,mype_mxtm,mype_mitm,&
...
! Declare passed variables
  integer(i_kind)                                  ,intent(in   ) :: mype,i_ps,i_uv,&
       i_t,i_q,i_pw,i_rw,i_dw,i_gps,i_sst,i_tcp,i_lag,i_gust,i_vis,&
       i_pblri,i_pblrf,i_pblsld,i_pblgld,i_pblrd,&
...
! Declare local variables
  ...
  integer(i_kind) numgrsq,numhgh,numgust,numvis,numpblri,numpblrf,numpblsld,numpblgld,numpblrd,&
                  numwspd10m,numuwnd10m,numvwnd10m
...
! Summary report for conventional pblsld
  if(mype==mype_pblsld) then
     if(first)then
        open(iout_pblsld)
     else
        open(iout_pblsld,position='append')
     end if

     numpblsld=nint(awork(5,i_pblsld))
     pw=zero ; pw3=zero
     nread=0
     nkeep=0
     do i=1,ndat
        if(dtype(i)== 'pblsld')then
           nread=nread+ndata(i,2)
           nkeep=nkeep+ndata(i,3)
        end if
     end do
     if(nkeep > 0)then
        mesage='current fit of conventional pblsld data, ranges in  m$'
        do j=1,nconvtype
           pflag(j)=trim(ioctype(j)) == 'pblsld'
        end do
        call dtast(bwork,1,pbotall,ptopall,mesage,jiter,iout_pblsld,pflag)

        numgross=nint(awork(6,i_pblsld))
        numfailqc=nint(awork(21,i_pblsld))
        if(numpblsld > 0)then
           pw=awork(4,i_pblsld)/numpblsld
           pw3=awork(22,i_pblsld)/numpblsld
        end if
        write(iout_pblsld,925) 'pblsld',numgross,numfailqc
     end if
     write(iout_pblsld,950) 'pblsld',jiter,nread,nkeep,numpblsld
     write(iout_pblsld,951) 'pblsld',awork(4,i_pblsld),awork(22,i_pblsld),pw,pw3

     close(iout_pblsld)
  end if

! Summary report for conventional pblgld
  if(mype==mype_pblgld) then
     if(first)then
        open(iout_pblgld)
     else
        open(iout_pblgld,position='append')
     end if

     numpblgld=nint(awork(5,i_pblgld))
     pw=zero ; pw3=zero
     nread=0
     nkeep=0
     do i=1,ndat
        if(dtype(i)== 'pblgld')then
           nread=nread+ndata(i,2)
           nkeep=nkeep+ndata(i,3)
        end if
     end do
     if(nkeep > 0)then
        mesage='current fit of conventional pblgld data, ranges in  m$'
        do j=1,nconvtype
           pflag(j)=trim(ioctype(j)) == 'pblgld'
        end do
        call dtast(bwork,1,pbotall,ptopall,mesage,jiter,iout_pblgld,pflag)

        numgross=nint(awork(6,i_pblgld))
        numfailqc=nint(awork(21,i_pblgld))
        if(numpblgld > 0)then
           pw=awork(4,i_pblgld)/numpblgld
           pw3=awork(22,i_pblgld)/numpblgld
        end if
        write(iout_pblgld,925) 'pblgld',numgross,numfailqc
     end if
     write(iout_pblgld,950) 'pblgld',jiter,nread,nkeep,numpblgld
     write(iout_pblgld,951) 'pblgld',awork(4,i_pblgld),awork(22,i_pblgld),pw,pw3

     close(iout_pblgld)
  end if

! Summary report for conventional pblrd
  if(mype==mype_pblrd) then
     if(first)then
        open(iout_pblrd)
     else
        open(iout_pblrd,position='append')
     end if

     numpblrd=nint(awork(5,i_pblrd))
     pw=zero ; pw3=zero
     nread=0
     nkeep=0
     do i=1,ndat
        if(dtype(i)== 'pblrd')then
           nread=nread+ndata(i,2)
           nkeep=nkeep+ndata(i,3)
        end if
     end do
     if(nkeep > 0)then
        mesage='current fit of conventional pblrd data, ranges in  m$'
        do j=1,nconvtype
           pflag(j)=trim(ioctype(j)) == 'pblrd'
        end do
        call dtast(bwork,1,pbotall,ptopall,mesage,jiter,iout_pblrd,pflag)

        numgross=nint(awork(6,i_pblrd))
        numfailqc=nint(awork(21,i_pblrd))
        if(numpblrd > 0)then
           pw=awork(4,i_pblrd)/numpblrd
           pw3=awork(22,i_pblrd)/numpblrd
        end if
        write(iout_pblrd,925) 'pblrd',numgross,numfailqc
     end if
     write(iout_pblrd,950) 'pblrd',jiter,nread,nkeep,numpblrd
    write(iout_pblrd,951) 'pblrd',awork(4,i_pblrd),awork(22,i_pblrd),pw,pw3
