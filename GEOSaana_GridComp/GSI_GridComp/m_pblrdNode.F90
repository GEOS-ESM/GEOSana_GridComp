...
!                .      .    .                                       .
! subprogram:	 module m_pblrdNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblrdNode (surface pblrd)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblrdNode
     !type(pblrd_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblrd residual
     real(r_kind)    :: err2   =0._r_kind       !  pblrd error squared
  ...
  public:: pblrdNode_typecast
  public:: pblrdNode_nextcast
        interface pblrdNode_typecast; module procedure typecast_ ; end interface
        interface pblrdNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblrdNode_appendto
        interface pblrdNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblrdNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblrdNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblrdNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblrdNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblrdNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblrdNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblrdNode),intent(in):: aNode
