...
real(r_kind),pointer,dimension(:,:)   :: rv_pblri=>NULL(),rv_pblrf=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_pblsld=>NULL(),rv_pblgld=>NULL(),rv_pblrd=>NULL()
...
call gsi_bundlegetpointer (grad%step(1),'pblsld',icpblsld,istatus)
call gsi_bundlegetpointer (grad%step(1),'pblgld',icpblgld,istatus)
call gsi_bundlegetpointer (grad%step(1),'pblrd',icpblrd,istatus)
...
!  Same one-to-one map for chemistry-vars; take care of them together
   ...
   if (icpblsld>0)then
      call gsi_bundlegetpointer (rval(jj),'pblsld' ,rv_pblsld, istatus)
      call gsi_bundleputvar ( wbundle, 'pblsld', rv_pblsld, istatus )
   end if
   if (icpblgld>0)then
      call gsi_bundlegetpointer (rval(jj),'pblgld' ,rv_pblgld, istatus)
      call gsi_bundleputvar ( wbundle, 'pblgld', rv_pblgld, istatus )
   end if
   if (icpblrd>0)then
      call gsi_bundlegetpointer (rval(jj),'pblrd' ,rv_pblrd, istatus)
      call gsi_bundleputvar ( wbundle, 'pblrd', rv_pblrd, istatus )
   end if
   if (icwspd10m>0) then
