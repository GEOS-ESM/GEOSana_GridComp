...
!                .      .    .                                       .
! subprogram:	 module m_pblgldNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblgldNode (surface pblgld)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblgldNode
     !type(pblgld_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblgld residual
     real(r_kind)    :: err2   =0._r_kind       !  pblgld error squared
  ...
  public:: pblgldNode_typecast
  public:: pblgldNode_nextcast
        interface pblgldNode_typecast; module procedure typecast_ ; end interface
        interface pblgldNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblgldNode_appendto
        interface pblgldNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblgldNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblgldNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblgldNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblgldNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblgldNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblgldNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblgldNode),intent(in):: aNode
