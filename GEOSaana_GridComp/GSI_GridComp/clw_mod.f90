...
! set routines used externally to public
  public :: calc_clw, ret_amsua, retrieval_mi
...
!   2014-03-29  j.jin - apply the SSMI procedure for TMI & GMI cloud retrieval using channels 2-9.
!   2014-09-03  j.jin - add obstype gmi for GMI 1CR data (for now).
!
!  input argument list:
!     nadir     - scan position
...
!     tmi       - flag for tmi  data
!     gmi       - flag for gmi  data channels 1-13 (made from GMI 1CR) 
...
! Declare local variables
  ...
  if (amsua .or. atms) then
    ...
    ! around -1.9C but we set the threshold at 1C to be safe.
     ...
     nchanl2 = 7_i_kind    ! cha 1&2 for TMI/GMI are not available for SSMI.
