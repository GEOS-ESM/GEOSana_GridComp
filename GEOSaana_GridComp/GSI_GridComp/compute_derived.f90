...
! Load guess gust, vis, pblh, lcbas, & cldch for use in limg, limv, limp, & liml.
  ...
  call gsi_bundlegetpointer (gsi_metguess_bundle(ntguessig),'pblsld',ptr2d,istatus)
  if (istatus==0) then
     do j=1,lon2
        do i=1,lat2
           pgues(i,j)=max(100.0_r_kind,ptr2d(i,j))
  ...
  call gsi_bundlegetpointer (gsi_metguess_bundle(ntguessig),'pblgld',ptr2d,istatus)
  if (istatus==0) then
     do j=1,lon2
        do i=1,lat2
           pgues(i,j)=max(100.0_r_kind,ptr2d(i,j))
  ...
  call gsi_bundlegetpointer (gsi_metguess_bundle(ntguessig),'pblrd',ptr2d,istatus)
  if (istatus==0) then
     do j=1,lon2
        do i=1,lat2
           pgues(i,j)=max(100.0_r_kind,ptr2d(i,j))
