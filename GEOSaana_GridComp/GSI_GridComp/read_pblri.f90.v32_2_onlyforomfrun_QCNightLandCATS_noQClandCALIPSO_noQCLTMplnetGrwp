...
!   2024-08     E.-G. Yang - added different obs error model for CALIPSO, CATS, MPLNET, and GRWP
!   2024-08     E.-G. Yang - changed local time QC for MPLNET and GRWP
!   2024-08     E.-G. Yang - changed obs error for CALIPSO (larger than before)
...
!     Initialize obs err parameters
      ...
      cerr1a = 200.0_r_kind ! raob, mplnet
...
!        Get information from surface file necessary for conventional data here
         ...
         if (kx == 891) then
            ...
            ! QC over land
            ! pblriosfc: 1=Land, 2=water, 3=ice and snow, 4=mixed (coastlines)
            !else if (pblriosfc /= 2) then ! pblriosfc = 2 (sfc_mask_cats(i)=17) : water
            !                            ! pblriosfc /= 2 : land, ice and snow
            !   pblriqm=10
            !   print*, "yeg_cats_qc land: 1: land, 3:ice,snow, pblriosfc=",pblriosfc
            end if
         ...
         if (kx == 890) then ! calipso
            ...
            ! QC over land
            ! pblriosfc: 1=Land, 2=water, 3=ice and snow, 4=mixed (coastlines)
            !else if (pblriosfc /= 2) then ! pblriosfc = 2 : water
            !                            ! pblriosfc /= 2 : land (no 3:ice_and_snow for calipso)
            !   pblriqm=10
            !   print*, "yeg_calipso_qc land: 1: land, pblriosfc=",pblriosfc
...
!        setup for sort of averaged obs 
         ...
         if (pblriob == r_missing) then
            ...
            if (nkx==120 .or. nkx==892) then ! RAOB or MPLNET
               if(pblriob <= cpblh1) then
                  pblrioe = cerr1a
               else if(pblriob > cpblh1 .and. pblriob < cpblh2) then
                  pblrioe = cerr1a + (pblriob-cpblh1)* &
                           (cerr2a-cerr1a)/(cpblh2-cpblh1)
               else 
                  pblrioe = cerr2a
               endif         
            else if (nkx==893) then ! GRWP
               if(pblriob <= cpblh1) then
                  pblrioe = cerr1b
               else if(pblriob > cpblh1 .and. pblriob < cpblh2) then
                  pblrioe = cerr1b + (pblriob-cpblh1)* &
                           (cerr2b-cerr1b)/(cpblh2-cpblh1)
               else
                  pblrioe = cerr2b
               endif
            else if (nkx==890) then ! CALIPSO
               if(pblriob <= cpblh1) then
                  pblrioe = cerr1c
               else if(pblriob > cpblh1 .and. pblriob < cpblh2) then
                  pblrioe = cerr1c + (pblriob-cpblh1)* &
...
!               !pblrioe=1.0_r_kind ! 1.2 ! 1.02_r_kind ! 0.5_r_kind ! 1.02_r_kind ! 0.96_r_kind
         ...
         if (nkx==892 .or. nkx==893) then ! MPLNET or GRWP
...
!            end if
         ! inflate obs error of MPLNET or GRWP according to averaged no. of obs in DA window
            pblrioe=pblrioe*sqrt(3.0_r_kind)
            if (nkx==892) print*, 'mplnet_oerror=',pblrioe
            if (nkx==893) print*, 'grwp_oerror=',pblrioe
         end if
