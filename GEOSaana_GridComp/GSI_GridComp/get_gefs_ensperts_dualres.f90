...
!   2014-11-30  todling - only calculate RH when needed
!                       - partially generalized to handle any control vector
...
!$$$ end documentation block
  ...
  real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig):: qs
  ...
  real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2):: z,sst2
...
! Allocate bundle to hold mean of ensemble members
  ...
  do m=1,ntlevs_ens
     ...
     if(istatus/=0) then
        write(6,*)' get_gefs_ensperts_dualres: trouble creating slot ',m,' for en_bar bundle'
        call stop2(999)
     endif
  end do

! Allocate bundle used for reading members
  call gsi_bundlecreate(en_read,grid_ens,'aux-ens-read',istatus,names2d=cvars2d,names3d=cvars3d)
  if(istatus/=0) then
  ...
  do m=1,ntlevs_ens
     ...
     do n=1,n_ens
       ...
       if (enspreproc) then
          ...
          call gsi_bundlegetpointer(en_read,'u' ,u ,ier);istatus=istatus+ier
          call gsi_bundlegetpointer(en_read,'v' ,v ,ier);istatus=istatus+ier
          ...
          call gsi_bundlegetpointer(en_read,'cw',cw,ier);istatus=istatus+ier
          if (istatus/=0) then
              if (mype==0) then
 ...
 103         format('ensmem',i3.3,'_f',i2.2,'.pe',i4.4)
          ...
          read(lunges,err=104) scr3; cw = scr3
          close(lunges)
          go to 105
 104      continue
          iret = -1
 ...
 106         format('sigf',i2.2,'_ens_mem',i3.3)
          ...
          if (use_gfs_ens) then
             ...
             call general_read_gfsatm(grd_ens,sp_ens,sp_ens,filename,mype,uv_hyb_ens,en_read,iret)
          else
...
! Compute RH, when needed
! Get 3d pressure field now on interfaces
       if (.not.q_hyb_ens) then
          call gsi_bundlegetpointer(en_read,'ps',ps,ier);istatus=ier
          call gsi_bundlegetpointer(en_read,'t' ,tv,ier);istatus=istatus+ier
          call gsi_bundlegetpointer(en_read,'q' ,q ,ier);istatus=istatus+ier
! When tv and ps are available
          if (ier==0) then
...
! get rid of read bundle
  call gsi_bundledestroy(en_read,istatus)
