...
!                .      .    .                                       .
! subprogram:	 module m_pblkhNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblkhNode (surface pblkh)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblkhNode
     !type(pblkh_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblkh residual
     real(r_kind)    :: err2   =0._r_kind       !  pblkh error squared
  ...
  public:: pblkhNode_typecast
  public:: pblkhNode_nextcast
        interface pblkhNode_typecast; module procedure typecast_ ; end interface
        interface pblkhNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblkhNode_appendto
        interface pblkhNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblkhNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblkhNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblkhNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblkhNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblkhNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblkhNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblkhNode),intent(in):: aNode
