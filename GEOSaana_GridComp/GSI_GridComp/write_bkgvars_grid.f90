...
subroutine write_crtmsfc_grid(a,b,c,fname,mype)
!$$$  subroutine documentation block
!
! subprogram:    write_crtmsfc_grid
!
!   prgrmmr:
!
! abstract:  help ensure crtm-surface fields are properly passed to GSI.
!
! program history log:
!   2024-11-11  todling -- add based on programs avalable here
!
!   input argument list:
!     mype     - mpi task id
!     a        -
...
!$$$
  ...
  real(r_kind),dimension(lat2,lon2),intent(in   ) :: a,b,c


  real(r_kind),dimension(nlat,nlon):: ag,bg,cg

  real(r_single),dimension(nlon,nlat):: a4,b4,c4
...
! gather stuff to processor 0
  call gather_stuff2(a,ag,mype,0)
  call gather_stuff2(b,bg,mype,0)
  call gather_stuff2(c,cg,mype,0)

  if (mype==0) then
     write(6,*) 'WRITE OUT SFC FIELDS'
! load single precision arrays
     do j=1,nlon
        do i=1,nlat
           a4(j,i)=ag(i,j)
...
! Create byte-addressable binary file for grads
     grdfile = trim(fname)//'.grd' 
     call baopenwt(22,trim(grdfile),iret)
     call wryte(22,4*nlat*nlon,a4)
     call wryte(22,4*nlat*nlon,b4)
     call wryte(22,4*nlat*nlon,c4)
     call baclose(22,iret)
...
!    Now create corresponding grads table file
     ...
     write(lu,'(a,2x,i4,2x,a,2x,f5.1,2x,f9.6)') 'XDEF',nlon, 'LINEAR',   0.0, 360./nlon
     write(lu,'(a,2x,i4,2x,a,2x,f5.1,2x,f9.6)') 'YDEF',nlat, 'LINEAR', -90.0, 180./(nlat-1.)
     write(lu,'(a,2x,i4,2x,a,1x,f8.3)')    'ZDEF',1, 'LEVELS', prs(1) ! prs is in mb
     write(lu,'(a,2x,i4,2x,a)')   'TDEF', 1, 'LINEAR 12:00Z04JUL1776 6hr' ! any date suffices
     write(lu,'(a,2x,i4)')        'VARS', 3
     write(lu,'(a,1x,2(i4,1x),a)') 'a',1,0,'a'
     write(lu,'(a,1x,2(i4,1x),a)') 'b',1,0,'b'
     write(lu,'(a,1x,2(i4,1x),a)') 'c',1,0,'c'
     write(lu,'(a)') 'ENDVARS'
     close(lu)
  ...
  return
end subroutine write_crtmsfc_grid
