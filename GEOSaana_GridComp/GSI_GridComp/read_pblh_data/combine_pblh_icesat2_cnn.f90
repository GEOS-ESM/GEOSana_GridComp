  program read_pblh_lidar_icesat2_cnn
...
!     integer(i_kind) :: iyear, imonth, idate, ihour, iminute
      ...
      integer(kind=1), allocatable,dimension(:) :: foldflag
      ...
      integer(kind=1), dimension(maxnum,ntime) :: sfoldflag
...
!        Read data
         ...
         ! Allocate memory
         allocate(PBL_z(nr), lat(nr), lon(nr), foldflag(nr))
         allocate(jdy(nr))

         ! Read PBL_z (float32)
         read(unit_number) PBL_z
         print *, "PBL_z read successfully."
         print *, "PBL_z(1)=",PBL_z(1)

         ! Read lat (float32)
         read(unit_number) lat
         print *, "lat read successfully."
         print *, "lat(1)=",lat(1)

         ! Read lon (float32)
         read(unit_number) lon
         print *, "lon read successfully."
         print *, "lon(1)=",lon(1)
         ...
         ! Read foldflag (integer 1byte)
         read(unit_number) foldflag
         print *, "foldflag read successfully."
         print *, "foldflag(1)=",foldflag(1)
...
!I2-CNN_PBL_X_20240805133732.dat
...
!        Sorting based on analysis window 
         do i = 1, nr
            ...
            if ( i>1 .and. int(jdy(i)) > int(jdy(i-1)) ) then 
...
!            print*, i, "obs_time=", obs_time
            ...
            ssec(ii,kk) = rsec(i) 
            spblh(ii,kk) = PBL_z(i)
            sfoldflag(ii,kk) = foldflag(i)

         end do

         deallocate(lat, lon, PBL_z, jdy, foldflag)
...
!     Write into netcdf files for each analysis window
      do i = 1, ntime
         ...
         write(filename2(i), '(A17,I10,A4)') 'icesat2_pblh_cnn.', ana_time(i), '.nc4'
...
!        if (ierr2 /= nf90_noerr) call handle_err(ierr2,"open")
         ...
         ierr2 = nf90_def_var(ncid2(i), 'Fold_Flag', NF90_BYTE, dimid1, varid11)
         ...
         ierr2 = nf90_put_att(ncid2(i), varid11, 'units', 'Cloud_fold_flag_0_no_folding_1_2_3_folding_was_indicated')
         ...
         ierr2 = nf90_put_var(ncid2(i), varid11, sfoldflag(1:kk,i))  

         ierr2 = NF90_CLOSE(ncid2(i))
      end do
  end program read_pblh_lidar_icesat2_cnn
  ...
  subroutine djul_day_inv(jday,hour,min,sec)
    ...
    real(kind=8),intent(in) :: jday     ! Fractional day-of-year input (e.g., 253.8365...)
    ! dtds CATS data has jday values like 253.8365, which is different from CNN CATS
    real(r_double) :: fractional_day      ! only the decimal part
    ...
    ! 1. Extract only the fractional part of the day
    ! int(jday) gives 253, so 253.8365 - 253 = 0.8365
    fractional_day = jday - int(jday)

    ! 2. Convert only that fraction into seconds
    secs = fractional_day * 86400.0      ! Convert jday to total seconds
    !secs = jday * 86400.0      ! Convert jday to total seconds

    ! 3. Calculate hour, min, and remaining seconds
