...
!$$$ end documentation block
  ...
  use hybrid_ensemble_parameters, only: use_gfs_ens,s_ens_v,sst_staticB
...
! real(r_kind),dimension(grd_ens%nlat,grd_ens%nlon):: sst_full,dum
  ...
  type(gsi_bundle),allocatable:: en_read(:)
...
! Allocate bundle used for reading members
  ...
  do n=1,n_ens
     call gsi_bundlecreate(en_read(n),en_perts(1,1)%grid,'aux-ens-read',istatus,names2d=cvars2d,names3d=cvars3d)
     if(istatus/=0) then
...
!  end do
   ...
   do n=n_ens,1,-1
      ...
      if(istatus/=0) then
         write(6,*)' in get_gefs_ensperts_dualres: trouble destroying en_read bundle'
                call stop2(999)
      endif
   end do
