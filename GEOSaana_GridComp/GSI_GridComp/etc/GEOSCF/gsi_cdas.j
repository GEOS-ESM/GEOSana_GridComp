...
#SBATCH --ntasks=320 --nodes=3 ###--ntasks-per-node=80
#SBATCH --job-name=CF2_gsi
#SBATCH --output=logs/CF2_gsi_%j
#SBATCH --error=logs/CF2_gsi_%j
...
#SBATCH --mail-user=viral.shah@nasa.gov
...
setenv EXPID CF2
setenv CORRTMPL cec_inst_3hr_glo_L576x361_v72
...
setenv  HLDDIR  $EXPDIR/holding_gsi
setenv  BKGDIR  $HLDDIR/bkg
setenv  RSTDIR  $EXPDIR/restarts_gsi
setenv  FVBUILD /discover/nobackup/projects/gmao/geos-cf-v2/CFv2_code/GEOSadas_t1/install_mil

setenv FVBIN          $FVROOT/bin
setenv RUN_CMDGSI      "$FVROOT/bin/esma_mpirun -np "

if (! -e $HLDDIR) mkdir -p $HLDDIR
...
if (! -e $HLDDIR/ana2inc ) mkdir -p $HLDDIR/ana2inc
if (! -e $HLDDIR/ods ) mkdir -p $HLDDIR/ods
if (! -e $HLDDIR/ana_stats ) mkdir -p $HLDDIR/ana_stats
#if (! -e $HLDDIR/diags_nc ) mkdir -p $HLDDIR/diags_nc
...
# Download obs files
echo "downloading OMI SO2 data from GESDISC"
set dummy = `ssh discover33 '/bin/bash /discover/nobackup/vshah5/cf2/CF2_hindcast/getobs.sh OMSO2' >& ${SCRDIR}/get_so2_obs.log`
#source $cmd &

echo "downloading OMI NO2 data from GESDISC"
set dummy = `ssh discover33 '/bin/bash /discover/nobackup/vshah5/cf2/CF2_hindcast/getobs.sh OMNO2' >& ${SCRDIR}/get_no2_obs.log`
#source $cmd &
...
#  /bin/cp -f  $EXPDIR/analyze/*.rc .
  ...
  # wait for 5 minutes, for files to be moved if they are being written to TSE_TMP
  sleep 300
  if ( ! -e ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4 ) then
    if ( -e ${RSTDIR}/codas_background_rst.${nymdc}_${hourc}00z.tar ) then
       ...
       if ( -e $EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4 ) then
        /bin/mv $EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4 ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4
       else
        echo "Warning: file not found: $EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4"
       endif
    endif 
  endif
...
#   @ NPES_GSI = $NX_GSI * $NY_GSI
   ...
   /bin/cp ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4 $EXPID.cbkg.eta.${nymdc}_${hourc}00z.nc4 
...
#   /bin/cp ${GEOSDIR}/$EXPID.bkg.sfc.corrector.${nymdc}_${hourc}00z.nc4  $EXPID.bkg.sfc.${nymdc}_${hourc}00z.nc4 
   ...
   /bin/cp ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdc}_${hourc}00z.nc4 ${BKGDIR}
...
#   /bin/mv satbias_pc.out satbias_pc
   ...
   /bin/cp ana.eta.nc4 $HLDDIR/ana2inc/$EXPID.ana.eta.${nymdm}_${hourm}00z.nc4
   /bin/cp inc.eta.nc4 $HLDDIR/ana2inc/$EXPID.inc.eta.${nymdm}_${hourm}00z.nc4
   ...
   if( 1 ) then
    ...
    /bin/cp $EXPID.ana_stats.log.${nymdm}_${hourm}z.txt $HLDDIR/ana_stats/$EXPID.ana_stats.log.${nymdm}_${hourm}z.txt
    ...
    foreach odsfile ($odsfiles)
     /bin/cp $odsfile $HLDDIR/ods
    end
    if ( -e ods.log ) then
     /bin/cp ods.log $HLDDIR/ods/ods.log.${nymdm}_${hourm}z
    endif
   else
   ...
   if ( 0 ) then
    if ( -e $HLDDIR/diags_nc/${nymdm}_${hourm}z ) then
     /bin/rm -r $HLDDIR/diags_nc/${nymdm}_${hourm}z
    endif
    mkdir -p $HLDDIR/diags_nc/${nymdm}_${hourm}z
    set pefiles = `/bin/ls -1 pe*.nc4`
    foreach pefile ($pefiles)
     /bin/mv $pefile $HLDDIR/diags_nc/${nymdm}_${hourm}z
    end
   endif
...
#   /bin/cp $outfile $EXPDIR/holding/ana_no2_after_gsi
 ...
 # Save background files at final step for next cycle
 # --------------------------------------------------
 if ( -e ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdf}_${hourf}00z.nc4 ) then
 if ( -e ${GEOSDIR}/$EXPID.bkg.eta.${nymdf}_${hourf}00z.nc4 ) then
 if ( -e ${GEOSDIR}/$EXPID.bkg.sfc.${nymdf}_${hourf}00z.nc4 ) then
   /bin/cp ${GEOSDIR}/$EXPID.$CORRTMPL.${nymdf}_${hourf}00z.nc4 .
   /bin/cp ${GEOSDIR}/$EXPID.bkg.eta.${nymdf}_${hourf}00z.nc4 .
   /bin/cp ${GEOSDIR}/$EXPID.bkg.sfc.${nymdf}_${hourf}00z.nc4 .
   ...
   tar cf ${RSTDIR}/codas_background_rst.${nymdf}_${hourf}00z.tar \
          $EXPID.bkg.eta.${nymdf}_${hourf}00z.nc4 $EXPID.bkg.sfc.${nymdf}_${hourf}00z.nc4 $EXPID.$CORRTMPL.${nymdf}_${hourf}00z.nc4
 endif
 endif
 endif
