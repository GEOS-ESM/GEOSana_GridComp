module pblkh_setup
  ...
  private
  public:: setup
        interface setup; module procedure setuppblkh; end interface

contains
subroutine setuppblkh(obsLL,odiagLL,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    setuppblkh    compute rhs for conventional surface pblkh
...
!$$$
  ...
  use m_pblkhNode, only: pblkhNode_appendto
...
! Declare local variables
  ...
  real(r_kind) pblkhges,dlat,dlon,ddiff,dtime,error
  ...
  real(r_kind) cg_pblkh,wgross,wnotgross,wgt,arg,exp_arg,rat_err2
  ...
  integer(i_kind) ier,ilon,ilat,ipblkh,id,itime,ikx,imaxerr,ilocaltime,iqc
...
! character(8),allocatable,dimension(:):: cdiagbuf
  ...
  type(pblkhNode),pointer:: my_head
  type(obs_diag),pointer:: my_diag
  type(obs_diags),pointer:: my_diagLL
  character(len=*),parameter:: myname='setuppblkh'

  real(r_kind),allocatable,dimension(:,:,:) :: ges_ps
  real(r_kind),allocatable,dimension(:,:,:) :: ges_z
  real(r_kind),allocatable,dimension(:,:,:) :: ges_pblkh

! equivalence(rstation_id,station_id)

  type(obsLList),pointer,dimension(:):: pblkhhead
  pblkhhead => obsLL(:)
...
!  index information for data array (see reading routine)
  ...
  ipblkh=5     ! index of pblkh observation
  id=6        ! index of station id
  itime=7     ! index of observation time in data array
  ikxx=8      ! index of ob type
  !imaxerr=9   ! index of pblkh max error
  ilocaltime=9   ! index of pblkh observation localtime
...
! Check for missing data  !need obs value and error
  do i=1,nobs
    if (abs(data(ipblkh,i)-bmiss) .lt. 10.0_r_kind .or. data(ipblkh,i)<zero)  then
       muse(i)=.false.
       data(ipblkh,i)=rmiss_single   ! for diag output
    end if
  end do
...
! Interpolate to get pblkh at obs location/time
     call tintrp2a11(ges_pblkh,pblkhges,dlat,dlon,dtime,hrdifsig,&
        mype,nfldsig)
...
! Adjust observation error
     ...
     write(6,*) 'setuppblkh nobs=',nobs,' rstation_id,lon,lat,loctime,itime,ibin,ipblkh,pblkhges,muse=',data(id,i),data(ilone,i),data(ilate,i),data(ilocaltime,i),data(itime,i),ibin,data(ipblkh,i),pblkhges,muse(i)

     ddiff=data(ipblkh,i)-pblkhges
...
!    Gross check using innovation normalized by error
     if (abs(data(ipblkh,i)-rmiss_single) >= tiny_r_kind ) then
...
!    Compute penalty terms (linear & nonlinear qc).
     ...
     if(luse(i))then
        ...
        if (vqc .and. cvar_pg(ikx) > tiny_r_kind .and. error > tiny_r_kind) then
           ...
           cg_pblkh=cvar_b(ikx)
           wgross = cg_term*cvar_pg(ikx)/(cg_pblkh*wnotgross)
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        allocate(my_head)
        call pblkhNode_appendto(my_head,pblkhhead(ibin))
...
!       print*, 'setuppblkh dataid,rstation_id,station_id=',data(id,i),rstation_id,station_id
...
! Write information to diagnostic file
  if(conv_diagsave) then
     ...
     if(binary_diag .and. ii>0)then
        write(7)'pblkh',nchar,nreal,ii,mype,ioff0
...
! Check to see if required guess fields are available
  ...
  call gsi_metguess_get ('var::pblkh' , ivar, istatus )
  proceed=proceed.and.ivar>0
...
!    get pblkh ...
     ...
     if (istatus==0) then
         if(allocated(ges_pblkh))then
            write(6,*) trim(myname), ': ', trim(varname), ' already incorrectly alloc '
            call stop2(999)
         endif
         allocate(ges_pblkh(size(rank2,1),size(rank2,2),nfldsig))
         ges_pblkh=0.
         print*, "yeg_setuppblkh_size: size(rank2,1),size(rank2,2)=",size(rank2,1),size(rank2,2)
         do j=1,size(rank2,2) ! lon
            do i=1,size(rank2,1) ! lat
               ges_pblkh(i,j,1)=rank2(i,j)
               !ges_pblkh(i,j,1)=log(max(0.1_r_kind,rank2(i,j)))
!               ges_pblkh(i,j,1)=log(max(1.0_r_kind,rank2(i,j)))
               print*, "yeg_settuppblkh L505:i,j=",i,j,", rank2(i,j)=",rank2(i,j),", ges_pblkh(i,j,1)=",ges_pblkh(i,j,1),", max(1.0_r_kind,rank2(i,j))=",max(1.0_r_kind,rank2(i,j))
            end do
         end do
         print*, "yeg_setuppblkh_init_vars: L508, rank2(10,10)=",rank2(10,10),",ges_pblkh(10,10,1)=",ges_pblkh(10,10,1)
         do ifld=2,nfldsig
            call gsi_bundlegetpointer(gsi_metguess_bundle(ifld),trim(varname),rank2,istatus)
            do j=1,size(rank2,2) ! lon
               do i=1,size(rank2,1) ! lat
                  ges_pblkh(i,j,ifld)=rank2(i,j)
                  !ges_pblkh(i,j,ifld)=log(max(0.1_r_kind,rank2(i,j)))
!                  ges_pblkh(i,j,ifld)=log(max(1.0_r_kind,rank2(i,j)))
                  print*, "yeg_settuppblkh L516:i,j,ifld=",i,j,ifld,", rank2(i,j)=",rank2(i,j),", ges_pblkh(i,j,ifld)=",ges_pblkh(i,j,ifld)
               end do
            end do
            print*, "yeg_setuppblkh_init_vars: L519, rank2(0,0)=",rank2(0,0),",ges_pblkh(0,0,ifld)=",ges_pblkh(0,0,ifld)
         enddo
     else
...
900  format('conv_pblkh_',i2.2,'.nc4')
...
!       rdiagbuf(6,ii)  = rmiss_single       ! observation pressure (hPa)
        ...
        rdiagbuf(17,ii) = data(ipblkh,i)      ! PBLH observation (K)
        rdiagbuf(18,ii) = ddiff              ! obs-ges used in analysis (K)
        rdiagbuf(19,ii) = data(ipblkh,i)-pblkhges! obs-ges w/o bias correction (K) (future slot)
  ...
  type(obs_diag),pointer,intent(in):: odiag
! Observation class
  character(7),parameter     :: obsclass = '   pblkh'
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           call nc_diag_metadata("Observation",                   data(ipblkh,i)     )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff             )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", data(ipblkh,i)-pblkhges )
  ...
  subroutine final_vars_
    ...
    if(allocated(ges_pblkh)) deallocate(ges_pblkh)
  end subroutine final_vars_

end subroutine setuppblkh
end module pblkh_setup
