...
!  Declare fields in file (should come from resource file)
   integer(i_kind),parameter:: no2d=5
   integer(i_kind),dimension(no2d)::iptr2d
   character(len=5), parameter :: ovars2d(no2d) = (/ 'ps   ', 'z    ', 'pblri', 'pblrf', 'pblkh'/)
...
!  PBLH
   ...
   call gsi_bundlegetpointer (pert,'pblkh',ipnt,istatus)
   if(istatus==0) then
      call gsi_bundlegetpointer (flds,'pblkh',optr2d,istatus)
      if(iamsingle) then
        optr2d=pert%r2(ipnt)%qr4
      else
        optr2d=pert%r2(ipnt)%qr8
      ...
      if(mype==0) write(6,*) myname,': pblkh ens member not in incoming pert'
      call stop2(999)
   endif
