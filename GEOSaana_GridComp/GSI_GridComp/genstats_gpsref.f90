...
!   2025-02-19  e yang   - changed the code for gps refractivity gradient DA
!
...
! module interface:
  ...
  use m_gpsrefNode, only: gpsref_ob_type => gpsrefNode
  ...
  !public:: gps_all_ob_type      ! currently not required to be public
    ...
    type gpsref_all_ob_type
      type(gpsref_all_ob_type),pointer :: llpoint => NULL()
      type(gpsref_ob_type),pointer :: mmpoint => NULL()
      ...
      character(len=40), dimension(10)  :: rdiag_extra_name
      integer(i_kind)                   :: n_rdiag_extra
    ...
    end type gpsref_all_ob_type

    type gpsref_all_ob_head
      integer(i_kind):: n_alloc=0
      type(gpsref_all_ob_type),pointer :: head => NULL()
    end type gpsref_all_ob_head

        ! Data objects:

  public:: gpsref_allhead
  public:: gpsref_alltail

        ! interfaces:

  public:: gpsrefStats_create
  public:: gpsrefStats_destroy

        interface gpsrefStats_create ; module procedure create_; end interface
        interface gpsrefStats_destroy; module procedure destroy_genstats_gpsref; end interface

  public:: gpsrefStats_genStats

        interface gpsrefStats_genStats; module procedure genstats_gpsref; end interface
...
! the node-growing, is remained in setupbend() and setupref() as is.
  ...
  type(gpsref_all_ob_head),dimension(:),pointer :: gpsref_allhead => null()
  type(gpsref_all_ob_head),dimension(:),pointer :: gpsref_alltail => null()

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='genstats_gpsref'
contains
  ...
  subroutine create_()
    ...
    ALLOCATE(gpsref_allhead(nobs_bins))
    ALLOCATE(gpsref_alltail(nobs_bins))
  end subroutine create_

  subroutine destroy_genstats_gpsref()
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$  end documentation block
    ...
    do ii=1,nobs_bins
       gpsref_alltail(ii)%head => gpsref_allhead(ii)%head
       do while (associated(gpsref_alltail(ii)%head))
          gpsref_allhead(ii)%head => gpsref_alltail(ii)%head%llpoint
          deallocate(gpsref_alltail(ii)%head,stat=istatus)
          if (istatus/=0) write(6,*)'DESTROY_GENSTATS_GPSREF: deallocate error for gpsref_all, istatus=',istatus
          gpsref_alltail(ii)%head => gpsref_allhead(ii)%head
       end do
    end do

    return
  end subroutine destroy_genstats_gpsref

subroutine genstats_gpsref(bwork,awork,toss_gps_sub,conv_diagsave,mype)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!   2025-02-10 eyang    - for refractivity gradient DA
!
!   input argument list:
...
! Declare local variables
  ...
  integer(i_kind) :: nnz, nind, i
  type(gpsref_ob_type), pointer:: gpsrefptr
  type(gpsref_all_ob_type), pointer:: gpsref_allptr
...
! Check to see if there are any profiles to process.  If none, return.
  if (nprof_gps==0) then
     if (mype==0) write(6,*)'GENSTATS_GPSREF:  no profiles to process (nprof_gfs=',nprof_gps,'), EXIT routine'
     return
  endif
...
! Get height of maximum bending angle
  ...
  DO ii=1,nobs_bins
     gpsref_allptr => gpsref_allhead(ii)%head
     do while (associated(gpsref_allptr))

!       Load local work variables
        kprof        = gpsref_allptr%kprof
        dtype        = gpsref_allptr%rdiag(20)
        dobs         = gpsref_allptr%rdiag(17)

        if (dtype == one .and. toss_gps(kprof) > zero .and. dobs == toss_gps(kprof)) then
           dobs_height_sub(kprof) = gpsref_allptr%rdiag(7)
        endif

        gpsref_allptr => gpsref_allptr%llpoint
...
!       Load local work variables
        ratio_errors = gpsref_allptr%ratio_err
        data_ier     = gpsref_allptr%obserr
        luse         = gpsref_allptr%luse
        kprof        = gpsref_allptr%kprof
        dtype        = gpsref_allptr%rdiag(20)
...
!       Load local work variables
        ...
        data_ier     = gpsref_allptr%obserr
        luse         = gpsref_allptr%luse
        muse         = gpsref_allptr%muse
        khgt         = gpsref_allptr%loc
        kprof        = gpsref_allptr%kprof
        dtype        = gpsref_allptr%rdiag(20)
        gpsrefptr       => gpsref_allptr%mmpoint
        if(muse .and. associated(gpsrefptr) .and. luse_obsdiag)then
           obsptr       => gpsrefptr%diags
        endif
...
!       term used in minimization
        ...
        if (super_gps(k,kprof)>tiny_r_kind) then
           ...
           if(conv_diagsave .and. luse) then
              if(gpsref_allptr%rdiag(16) >tiny_r_kind) gpsref_allptr%rdiag(16)=ratio_errors*data_ier
           endif
 
!          Adjust error ratio for observations used in inner loop
           if (associated(gpsrefptr)) then
              gpsrefptr%raterr2 = ratio_errors **2
        ...
        if(dtype == zero) then !refractivity
          if (rhgt<=toss_gps(kprof)) then
             if(ratio_errors*data_ier > tiny_r_kind) then ! obs was good
                if (luse) then
                   if(conv_diagsave) then
                      gpsref_allptr%rdiag(10) = four
                      gpsref_allptr%rdiag(12) = -one
                      gpsref_allptr%rdiag(16) = zero
                      if(lobsdiagsave) gpsref_allptr%rdiag(mreal+jiter) = -one
                   endif
                   elat         = gpsref_allptr%rdiag(3)
                   if(elat > r20) then
...
!       profile is below platform specific threshold height.
        if(regional) then
           ...
           if(ratio_errors*data_ier > tiny_r_kind) then
             if(dtype==zero) then !refractivity
                satid        = gpsref_allptr%rdiag(1)
                if((satid==41).or.(satid==722).or.(satid==723).or.(satid==4).or.(satid==786).or. &
                   (satid==3).or.(satid==5)) then
...
!       Compute penalty terms
        ...
        if(luse)then
           val          = gpsref_allptr%dataerr
           val2     = val*val
           exp_arg  = -half*val2
           rat_err2 = ratio_errors**2
           data_ipg     = gpsref_allptr%pg
           if (data_ipg > tiny_r_kind) then
              data_ib      = gpsref_allptr%b
           ...
           if(conv_diagsave) gpsref_allptr%rdiag(13) = wgt/wgtlim
...
!             Accumulate values for penalty and data count
           ...
           data_ikx     = gpsref_allptr%type
           ikx          = nint(data_ikx)
           pressure     = gpsref_allptr%rdiag(6)
           data_rinc    = gpsref_allptr%rdiag(5)*scale
...
!       Transfer diagnostic information to output arrays
        if(conv_diagsave .and. luse) then
           icnt=icnt+1
           cdiag(icnt) = gpsref_allptr%cdiag
           do j=1,nreal
              sdiag(j,icnt)= gpsref_allptr%rdiag(j)
           enddo
        endif
...
! If requested, write information to diagnostic file
  if(conv_diagsave) then
    ...
    if (binary_diag .and. icnt > 0)then
        nchar = 1
        write(7)'gpsref',nchar,nreal,icnt,mype,ioff ! is it right? yzhu
        write(7)cdiag,sdiag
...
!     write(string,900) jiter ! commented out (eyang)
!900  format('conv_gps_',i2.2,'.nc4') ! commented out (eyang)
     !----------------------
     ! conv_gps_ref (yzhu eyang)
     !----------------------
     write(string,900) jiter
900  format('conv_gps_ref_',i2.2,'.nc4') ! (eyang)
     !write(string, '(A, I2.2, A)') 'conv_gps_',jiter,'.nc4'
     !print*, 'yeg genstats_gpsref L749 trim(string)=',trim(string)
...
! 2025-02-12 E. Yang   - refractivity gradient
! 2025-03-05 E. Yang   - add model_sfc_type for idomsfc
...
! Observation class
  character(7),parameter     :: obsclass = ' gpsref' ! how to change? yzhu

  if (gpsref_allptr%rdiag(16) /= 0.0 )  obserr = 1.0/gpsref_allptr%rdiag(16)

           said     = gpsref_allptr%rdiag(1)
           profid   = gpsref_allptr%rdiag(2)
           siid     = gpsref_allptr%rdiag(29)
           sclf     = gpsref_allptr%rdiag(26)
           preqc    = gpsref_allptr%rdiag(10)
           ascd     = gpsref_allptr%rdiag(27)
           refid    = gpsref_allptr%rdiag(25)
           ogce     = gpsref_allptr%rdiag(28)
           ...
           call nc_diag_metadata("time",                        sngl(gpsref_allptr%rdiag(8)) )
           !   call nc_diag_metadata("impact_height",            sngl(gpsref_allptr%rdiag(7)) )
           !   call nc_diag_metadata("impact_parameter",         sngl(gpsref_allptr%rdiag(7)) + sngl(gps_allptr%rdiag(24)) )
           call nc_diag_metadata("Forecast_Refractivity",          sngl(gpsref_allptr%rdiag(23)) ) ! eyang
           !   call nc_diag_metadata("geoid_height_above_reference_ellipsoid", sngl(gpsref_allptr%rdiag(23)) )
           !   call nc_diag_metadata("earth_radius_of_curvature",              sngl(gpsref_allptr%rdiag(24)) )
           call nc_diag_metadata("Sat_Azimuth_Angle",           sngl(gpsref_allptr%rdiag(30)) )

!          geovals
           call nc_diag_metadata("surface_altitude",           sngl(gpsref_allptr%rdiag(9)) )
           call nc_diag_metadata("surface_geopotential_height",sngl(gpsref_allptr%rdiag(9)) )
           if (wrtgeovals) then
              call nc_diag_data2d("air_temperature",              sngl(gpsref_allptr%tsenges) )
              call nc_diag_data2d("virtual_temperature",          sngl(gpsref_allptr%tvirges) )
              call nc_diag_data2d("specific_humidity",            sngl(gpsref_allptr%sphmges) )
              call nc_diag_data2d("geopotential_height",          sngl(gpsref_allptr%hgtlges) )
              call nc_diag_data2d("geopotential_height_levels",   sngl(gpsref_allptr%hgtiges) )
              call nc_diag_data2d("atmosphere_pressure_coordinate_interface",  sngl(gpsref_allptr%prsiges) )
              call nc_diag_data2d("atmosphere_pressure_coordinate",            sngl(gpsref_allptr%prslges) )
           endif
!----------------
! The following is taken from GEOS-5.29
           call nc_diag_metadata("Station_ID",                            gpsref_allptr%cdiag             )
           call nc_diag_metadata("Observation_Class",                     obsclass                     )
           obstype    = gpsref_allptr%rdiag(1) 
           obssubtype = gpsref_allptr%rdiag(2)
           call nc_diag_metadata("Observation_Type",                      obstype                      )
           call nc_diag_metadata("Observation_Subtype",                   obssubtype                   )
           call nc_diag_metadata("Latitude",                              sngl(gpsref_allptr%rdiag(3))    )
           call nc_diag_metadata("Longitude",                             sngl(gpsref_allptr%rdiag(4))    )
           !call nc_diag_metadata("Incremental_Bending_Angle",          sngl(gpsref_allptr%rdiag(5))    )
           call nc_diag_metadata("Pressure",                              sngl(gpsref_allptr%rdiag(6))    )
           call nc_diag_metadata("Height",                                sngl(gpsref_allptr%rdiag(7))    )
           call nc_diag_metadata("Time",                                  sngl(gpsref_allptr%rdiag(8))    )
           call nc_diag_metadata("Model_Elevation",                       sngl(gpsref_allptr%rdiag(9))    )
           call nc_diag_metadata("Setup_QC_Mark",                         sngl(gpsref_allptr%rdiag(10))   )
           call nc_diag_metadata("Prep_Use_Flag",                         sngl(gpsref_allptr%rdiag(11))   )
           call nc_diag_metadata("Analysis_Use_Flag",                     sngl(gpsref_allptr%rdiag(12))   )
           call nc_diag_metadata("Nonlinear_QC_Rel_Wgt",                  sngl(gpsref_allptr%rdiag(13))   )
           call nc_diag_metadata("Errinv_Input",                          sngl(gpsref_allptr%rdiag(14))   )
           call nc_diag_metadata("Errinv_Adjust",                         sngl(gpsref_allptr%rdiag(15))   )
           call nc_diag_metadata("Errinv_Final",                          sngl(gpsref_allptr%rdiag(16))   )
           call nc_diag_metadata("Observation",                        sngl(gpsref_allptr%rdiag(24))   ) ! ref gradient
           call nc_diag_metadata("Observation_Refractivity",           sngl(gpsref_allptr%rdiag(17))   ) ! ref
           !call nc_diag_metadata("Observation",                        sngl(gpsref_allptr%rdiag(17))   ) 
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",        sngl(gpsref_allptr%rdiag(5))   ) ! ref gradient
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted",      sngl(gpsref_allptr%rdiag(5))   )
           call nc_diag_metadata("Forecast_adjusted",                  sngl(gpsref_allptr%rdiag(25))  )
           call nc_diag_metadata("Forecast_unadjusted",                sngl(gpsref_allptr%rdiag(25)) )
           call nc_diag_metadata("Model_Sfc_Type",                     sngl(gpsref_allptr%rdiag(26)) )
           !call nc_diag_metadata("Obs_Minus_Forecast_adjusted",        sngl(gpsref_allptr%rdiag(17))*sngl(gpsref_allptr%rdiag(5)) )
           !call nc_diag_metadata("Obs_Minus_Forecast_unadjusted",      sngl(gpsref_allptr%rdiag(17))*sngl(gpsref_allptr%rdiag(5)) )
           !call nc_diag_metadata("Forecast_adjusted", sngl(gpsref_allptr%rdiag(17)-(gpsref_allptr%rdiag(17)*gpsref_allptr%rdiag(5))))
           !call nc_diag_metadata("Forecast_unadjusted", sngl(gpsref_allptr%rdiag(17)-(gpsref_allptr%rdiag(17)*gpsref_allptr%rdiag(5))))
           call nc_diag_metadata("GPS_Type",                              sngl(gpsref_allptr%rdiag(20))   )
           call nc_diag_metadata("Vertical_Grid_Location",                sngl(gpsref_allptr%rdiag(19))   )
           call nc_diag_metadata("Temperature_at_Obs_Location",           sngl(gpsref_allptr%rdiag(18))   )
           call nc_diag_metadata("Specific_Humidity_at_Obs_Location",     sngl(gpsref_allptr%rdiag(21))   )

           if (save_jacobian) then
              call readarray(dhx_dx, gpsref_allptr%rdiag(ioff+1:nreal))
           ...
           do i=1, gpsref_allptr%n_rdiag_extra
              call nc_diag_metadata(trim(gpsref_allptr%rdiag_extra_name(i)), sngl(gpsref_allptr%rdiag_extra_val(i)))
           enddo    
...
end subroutine genstats_gpsref

end module m_gpsrefStats
