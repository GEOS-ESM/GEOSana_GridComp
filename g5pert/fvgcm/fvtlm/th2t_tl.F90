...
!BOP
!
! !ROUTINE: th2t_tl --- Convert (vir.) pot. temp to (vir.) temp (TL-only)
!
!
! !INTERFACE:
!
subroutine th2t_tl ( delp, delp_tl, pt, pt_tl )
...
do k = 1, km
   do j = js2g0, jn2g0
      do i = 1, im
         pt_tl(i,j,k) = pt(i,j,k)*pkz_tl(i,j,k) + pkz(i,j,k)*pt_tl(i,j,k)
      enddo
   enddo
   if ( jfirst==1 ) then
      j = 1
