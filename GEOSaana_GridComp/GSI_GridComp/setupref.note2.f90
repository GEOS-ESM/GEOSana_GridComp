...
!-------------------------------------------------------------
! a) read obs assigned to given mpi task (geographic region)
!-------------------------------------------------------------
...
!       Convert observation height (in dpres) from meters to grid relative units
        ...
        dpres=hob ! interface levels
        dpresl(i)=hobl ! midpoint layers
        data(ihgt,i)=dpres
...
! d) load weight and innovation arrays used in minimization
!-------------------------------------------------------------
! TL = partial derivative = linearized forward operator
! dH/dx -> dH/dT, dH/dq
...
! terms, and accumulate statistics.
  if(last_pass) then
     do i=1,nobs-1
        ...
        ! SR-likely conditions below 3 km 
        if(r1em3*rdiagbuf(7,i) <= 3.0_r_kind) then ! check for SR-likely conditions below 3 km
            ...
            if( kprof == jprof .and. .not. qcfail(i) .and. qcfail_loc(i) == zero .and. qcfail_loc(i+1) == zero .and. qcfail_gross(i) == zero)then
                ...
                ! rdiagbuf(17,i) <- refractivity obs
                ! rdiagbuf(5,i) <- (O-F)/O (refractivity) (X100 %)
                ! rdiagbuf(7,i) <- elev (height in meters)
                ! crit_grad <- 157
...
!          Set (i,j,k) indices of guess gridpoint that bound obs location
           ...
           !data(ihgt,i): observation height (in dpres) converted from meters to grid relative units
           dpres=data(ihgt,i)
           k=dpres
           k1=min(max(1,k),nsig)
           k2=max(1,min(k+1,nsig))
           ! Jacobian w.r.t. T and q = dH/Dt, dH/dq
           ! TL
           ! obs vertical location between k1 and k2
