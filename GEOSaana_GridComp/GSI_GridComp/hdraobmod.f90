...
! set subroutines and functions to public
  public :: read_hdraob,binomial_coeffs
...
contains
  ...
  ! formula: n choose k = n!/(k! (n-k)!)
           ...
           real(r_double) p,q,t1,t2
           if(k.eq.n)nck=1
           if(k.eq.0)nck=1
           if(k.eq.1)nck=n
           if((k.ne.1).and.(k.ne.n).and.(k.ne.0))then 
               if(n.lt.21)then
               nck=product((/(ii,ii=1_i_llong,int(n,kind=i_llong),1_i_llong)/))&
                       /(product((/(ii,ii=1_i_llong,int(k,kind=i_llong),1_i_llong)/))&
                       *product((/(ii,ii=1_i_llong,int((n-k),kind=i_llong),1_i_llong)/)))
               else
                    !use Stirling approximation if n is large enough to cause overflow errors
                    p=real(k,kind=r_double)/real(n,kind=r_double)
                    q=1_r_double-p
                    t1=(1_r_double/sqrt(7_r_double*real(n,kind=r_double)*p*q))
                    t2=(((p**p)*(q**q))**(-n))
                    nck=int(t1*t2,kind=i_llong)
               end if 
...
!    filter size must be odd to ensure that kernal is centered on a particular level
     ...
     integer(i_kind) :: nn
     integer(i_llong) :: numerator 
     numerator=2_i_llong**(filter_size-1)
     write(6,*) 'numerator: ',numerator
     do nn=0,(filter_size-1)
         write(6,*) 'coef: ',nn,'val: ',real(nchoosek((filter_size-1),nn),r_double)

         coeffs(nn+1)=(real(nchoosek((filter_size-1),nn),r_double)/numerator)
     end do
     !normalize coeffs 
     coeffs=coeffs/sum(coeffs)
     return      
   end subroutine binomial_coeffs
...
! Declare local variables
  ...
  integer(i_kind),dimension(nsig):: sigma_count,previous_center ! hd raob thin/superob
  integer(i_kind) mlvl ! hd raob thin/superob
  integer(i_kind) bc,ki,filter_buff,iout_startk,iout_endk,filter_window_active ! hd raob smooth 
  real(r_kind) smooth_val,smooth_val2,delta_t,avg_tint! hd raob smooth 
  ...
  real(r_kind),dimension(nsig):: sigma_avg,sigma_avg2,pdistance ! hd raob thin/superob 
  real(r_kind),dimension(nsig-1):: dpres
  real(r_kind),dimension(maxlevs)::plevs
  real(r_kind),allocatable,dimension(:,:):: cdata_all,cdata_out
  real(r_kind) :: missing
  real(r_kind),allocatable,dimension(:):: coefs ! hd raob smooth
...
!          Extract type, date, and location information
           ...
           if(thin_flg.or.superob_flg)then !initialize sigma count and avg arrays for thinning/superobbing
                sigma_count= 0  
                sigma_avg= zero 
                sigma_avg2= zero 
                pdistance= 9999._r_kind
                previous_center=1
                errout=1._r_kind
                
           end if 
           !initialize smooth start stop iouts
           iout_startk=ndata+1
...
!             Temperature
              if(tob) then
                 !reject descending obs above 150 mb (high fallspeed can cause large temp bias) 
              ...
              if((thin_flg.or.superob_flg).and.(tob.or.qob.or.uvob).and.(usage<100._r_kind))then !thinning/supperobing application
                 ...
                 if(abs(presl(mlvl)-plevs(k)).lt.pdistance(mlvl))then
                         ...
                         !set previous closest ob to unused
                               ...
                               previous_center(mlvl)=iout   
                         end if 
                         pdistance(mlvl)=abs(presl(mlvl)-plevs(k))
                         previous_center(mlvl)=iout
                 else
                         !set observations further from presl center to unused 
                         if(tob)cdata_all(12,iout)=108._r_kind
                         if(qob)cdata_all(13,iout)=108._r_kind
                         if(uvob)cdata_all(14,iout)=108._r_kind
                 end if 
                 
                 if(superob_flg)then
                 !If we're superobbing then add observation value into the layer average  
                      if(tob.or.qob)sigma_avg(mlvl)=sigma_avg(mlvl)+cdata_all(5,iout)
                      if(uvob)sigma_avg(mlvl)=sigma_avg(mlvl)+cdata_all(6,iout)
                      if(uvob)sigma_avg2(mlvl)=sigma_avg2(mlvl)+cdata_all(7,iout)
                      if(k.eq.levs)then
                              do ki=1,nsig
                                  ...
                                  if(tob)then
                                     ...
                                     cdata_all(7,previous_center(ki))=(sigma_avg2(ki)/sigma_count(ki))
...
!    End k loop over levs
           ...
           iout_endk=ndata !get size of profile for smoothing

           if(smooth_flg.and.(.not.thin_flg).and.(.not.superob_flg).and.(iout_endk.gt.1).and.(qob.or.tob.or.uvob))then
! start smoothing operations
! calculate average time interval in profile, adjust filter window 
...
! smooth hd raob entries using a binomial filter of size: filter_window_active 
! check that filter window is odd, if not then add one
                if(MOD(filter_window_active,2).eq.0)filter_window_active=filter_window_active+1
! Reset filter window if larger than 61 to stay within safe range for binomial coeffs 
                if(filter_window_active.gt.61)filter_window_active=61
                if(print_verbose)write(6,*)'new filter window: ',filter_window_active

! initialize coefficients for binomial filter 
                allocate(coefs(filter_window_active))
                ...
                filter_buff=(filter_window_active+1)/2
                LOOP_K_SMOOTH: do ki=(iout_startk-1+filter_buff),(iout_endk-filter_buff)
! apply binomial filter 

                   smooth_val=zero
                   smooth_val2=zero
                   LOOP_APPLY: do bc=1,filter_window_active
                      if(tob.or.qob)smooth_val=smooth_val+(coefs(bc)*cdata_all(5,ki+bc-filter_buff))
                      if(uvob)then
                        smooth_val=smooth_val+(coefs(bc)*cdata_all(6,ki+bc-filter_buff))
                        smooth_val2=smooth_val2+(coefs(bc)*cdata_all(7,ki+bc-filter_buff))
                      end if
                   end do LOOP_APPLY
                   if(print_verbose)write(6,*)'at iout: ',ki,'replacing: ',cdata_all(5,ki),'with: ',smooth_val
                   if(tob)cdata_all(5,ki)=1._r_double*smooth_val
                   if(qob)cdata_all(5,ki)=smooth_val
                   if(uvob)then
                        cdata_all(6,ki)=smooth_val
                        cdata_all(7,ki)=smooth_val2
                   end if

                end do LOOP_K_SMOOTH
                deallocate(coefs)
             end if
           end if
! end smoothing/thinning
        end do loop_readsb
