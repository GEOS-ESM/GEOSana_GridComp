module gsi_pblrdOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_pblrdOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for pblrdNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: pblrdOper      ! data stracture

  type,extends(obOper):: pblrdOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_pblrdOper'
  type(pblrdNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(pblrdOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intpblrdmod, only: intjo => intpblrd
    ...
    implicit none
    class(pblrdOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stppblrdmod, only: stpjo => stppblrd
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
