...
!   5. Convert temperature to virtual temperature (do not need this step)
...
!   2024-09-13 eyang    - add Setup_QC_Mark to check if T is virtual or sensible
!                       - fixed error for virtual temperature
!
! usage:
!
...
!  Declare local variables
   ...
   real(r_double),allocatable,dimension(:)  :: setup_qc_mark_t! tvflg, qtflg (virtual temperature flag) -> 0: virtual T
   real(r_double),allocatable,dimension(:)  :: setup_qc_mark! raob
   ...
   real(r_double),allocatable,dimension(:)  :: raob_thetav ! potential temperature
...
!   real(r_kind)   :: geom_alti
   ...
   integer(i_kind) :: varid38,varid39
...
!  Temperature
   ...
   call read_diag(infile,obsvar,obs_type_t,lat_t,lon_t,stn_elev_t,pressure_t,&
        height_t,prep_qc_mark_t,obs_t,time_t,errinv_input_t,sid_t,date_time_t,setup_qc_mark_t)
...
!-----------------------------
   ...
   allocate(lat(n_raob_t), lon(n_raob_t), stn_elev(n_raob_t), pressure(n_raob_t), &
      height(n_raob_t), prep_qc_mark(n_raob_t), setup_qc_mark(n_raob_t), &
   ...
   deallocate(obs_type_t,lat_t,lon_t,stn_elev_t,pressure_t,height_t,prep_qc_mark_t,obs_t,time_t,sid_t, &
         errinv_input_t,setup_qc_mark_t)
...
!         timeuv(j)=time_uv(i)-3
 ...
 !------------------------------------------------------------------
   ...
   ! Temperature
   obsvar='t'
   call sorting(obsvar,lat,lon,stn_elev,pressure,height,prep_qc_mark,time,sid,raob_sid_list,sid_plevels_t,sid_plevels_t_500,raob_t,setup_qc_mark)
   print*, "After sorting T sid=",sid
   print*, "After sorting T setup_qc_mark=",setup_qc_mark
...
!------------------------------------------------------------------
 ...
 !------------------------------------------------------------------
 ! 5. Convert temperature to virtual temperature (depending on setup_qc_flag)
 ! 6. Convert virtual temperature to virtual potential temperature
 ! 8. Convert Pressure -> geopotential height 
 !------------------------------------------------------------------
   allocate(height_converted(n_raob_t))
   !allocate(height_converted(n_raob_t), geo_altitude_converted(n_raob_t))
   allocate(raob_tv(n_raob_t), raob_thetav(n_raob_t))
   ...
   iloop_convert: do i = 1, size(raob_t)
      ...
      !  w = mixing ratio
      !------------------------------------------------------------------
      if (setup_qc_mark(i)==0) then ! virtual temperature
         raob_tv(i) = raob_t(i)
      else ! sensible temperature
...
!     print*, "                raob_thetav=",raob_thetav(i)
   ...
   deallocate(raob_tv,raob_thetav)
...
!*****************************************************************
   subroutine sorting(obsvar,lat,lon,stn_elev,pressure,height,prep_qc_mark,time,sid,raob_sid_list,sid_plevels,sid_plevels_500,raob,raob_v_or_setup_qc_mark)
...
!     sid       - station id
!     raob_v_or_setup_qc_mark    - v observation or setup_qc_mark - tvflg, qtflg (virtual temperature flag) -> 0: virtual T (optional)
!
...
!  Declare passed variables
   ...
   real(r_double),allocatable,dimension(:),optional,intent(inout)  :: raob_v_or_setup_qc_mark
   ...
   real(r_double),allocatable,dimension(:) :: setup_qc_mark_so
...
!  newly sorted
   ...
   real(r_double),allocatable,dimension(:)  :: setup_qc_mark1
...
!------------------------------------------------------------------
   ...
   if (obsvar=='t') allocate(setup_qc_mark1(size(sid)))
   ...
   do i = 1, number_of_sid ! the number of raob_sid_list
      ...
      allocate(lat_so(np), lon_so(np), stn_elev_so(np), pressure_so(np), &
         ...
         sid_so(np))
      if (obsvar=='t') allocate(setup_qc_mark_so(np))
      ...
      do j = 1, size(sid)
         ...
         if (sid(j) == raob_sid_list(i)) then ! for the same sid
            ...
            if (obsvar=='t') setup_qc_mark_so(k)=raob_v_or_setup_qc_mark(j)
            raob_so(k)=raob(j)
            if (obsvar=='uv') raob_v_so(k)=raob_v_or_setup_qc_mark(j)
            time_so(k)=time(j)
            sid_so(k)=sid(j)
            k = k + 1
      ...
      do k = 1, np-1 ! except for the last
         ...
         if (obsvar=='t') call Swap(setup_qc_mark_so(k), setup_qc_mark_so(location)) ! swap this and the maximum
      ...
      do k = 1, np 
         ...
         if (obsvar=='t') setup_qc_mark1(indexx+k)=setup_qc_mark_so(k)
      ...
      deallocate(lat_so, lon_so, stn_elev_so, pressure_so, &
         ...
         sid_so)
      if (obsvar=='t') deallocate(setup_qc_mark_so)
   ...
   do i = 1, size(sid)
      ...
      raob(i)=raob1(i)
      if (obsvar=='t') raob_v_or_setup_qc_mark(i)=setup_qc_mark1(i)
      if (obsvar=='uv') raob_v_or_setup_qc_mark(i)=raob_v1(i)
      time(i)=time1(i)
      sid(i)=sid1(i)
   ...
   if (obsvar=='t') deallocate(setup_qc_mark1)
...
!----------------------------------------------------------------------
   subroutine read_diag(infile,obsvar,obs_type,lat,lon,stn_elev,pressure,&
     height,prep_qc_mark,obs,time,errinv_input,sid,date_time,obs_v_or_setup_qc_mark)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!     prep_qc_mark - qc flag
!     obs       - observation
!     obs_v_or_setup_qc_mark - v obs or setup_qc_mark (optional)
...
!  Declare passed variables
   ...
   real(r_double),allocatable,dimension(:),optional,intent(out) :: obs_v_or_setup_qc_mark
...
!--------------------------
   ...
   if (obsvar=='t') allocate(obs_v_or_setup_qc_mark(nobs))
   allocate(time(nobs), errinv_input(nobs))
   allocate(sid_org(nobs, strlen), sid(nobs), obs_type(nobs))
   if (obsvar=='uv') allocate(obs_v_or_setup_qc_mark(nobs))
...
!  Observation 
   if (obsvar == 'uv') then
      ...
      if (ierr == nf90_noerr) ierr = NF90_GET_VAR(ncid,varid80,obs_v_or_setup_qc_mark)
      ...
      if (obsvar=='t') then
      !  Setup_QC_Mark -> tvflg, qtflg (virtual temperature flag) -> 0: virtual T
         ierr = NF90_INQ_VARID(ncid,'Setup_QC_Mark',varid80)
         if (ierr == nf90_noerr) ierr = NF90_GET_VAR(ncid,varid80,obs_v_or_setup_qc_mark)
      end if
