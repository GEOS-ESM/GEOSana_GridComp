...
!
subroutine vpol5_tl( u, u_tm, v, v_tm, im, jm, coslon, sinlon, cosl5, sinl5, jfirst, jlast )
!******************************************************************
!******************************************************************
...
if (jfirst .eq. 1) then
  do i = 1, im
    uasp_tl(i) = u_tm(i,3)+u_tm(i,2)
    uasp(i) = u(i,2)+u(i,3)
  end do
  do i = 1, im-1
    vasp_tl(i) = v_tm(i+1,2)+v_tm(i,2)
    vasp(i) = v(i,2)+v(i+1,2)
  end do
  vasp_tl(im) = v_tm(im,2)+v_tm(1,2)
  ...
  vs = vs*r2im
  do i = 1, imh
    v_tm(i,1) = us_tl*cosl5(i)-vs_tl*sinl5(i)
    v(i,1) = us*cosl5(i)-vs*sinl5(i)
    v_tm(i+imh,1) = -v_tm(i,1)
    v(i+imh,1) = -v(i,1)
  end do
endif
if (jlast .eq. jm) then
  do i = 1, im
    uanp_tl(i) = u_tm(i,jm-1)+u_tm(i,jm)
    uanp(i) = u(i,jm-1)+u(i,jm)
  end do
  do i = 1, im-1
    vanp_tl(i) = v_tm(i+1,jm-1)+v_tm(i,jm-1)
    vanp(i) = v(i,jm-1)+v(i+1,jm-1)
  end do
  vanp_tl(im) = v_tm(im,jm-1)+v_tm(1,jm-1)
  ...
  vn = vn*r2im
  do i = 1, imh
    v_tm(i,jm) = (-(un_tl*cosl5(i)))-vn_tl*sinl5(i)
    v(i,jm) = (-(un*cosl5(i)))-vn*sinl5(i)
    v_tm(i+imh,jm) = -v_tm(i,jm)
    v(i+imh,jm) = -v(i,jm)
  end do
endif

end subroutine vpol5_tl
