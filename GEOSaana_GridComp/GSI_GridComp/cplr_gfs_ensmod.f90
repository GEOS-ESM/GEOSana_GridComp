...
!!  read ensembles
    ...
    write(filename,22) trim(adjustl(ensemble_path)),ens_fmnlevs(ntindex)/60,mas
...
!$$$
    ...
    if ( member == 0 ) then
       write(filename,12) trim(adjustl(ensemble_path)),ens_fmnlevs(ntindex)/60
    else
       write(filename,22) trim(adjustl(ensemble_path)),ens_fmnlevs(ntindex)/60,member
    endif
...
!$$$
    ...
    write(filename,13) trim(adjustl(ensemble_path)),ens_fmnlevs(ntindex)/60,member
