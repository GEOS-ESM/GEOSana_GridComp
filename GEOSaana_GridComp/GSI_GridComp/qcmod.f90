...
!  Reject some obs at the scan edges if use_edges=True which means these obs
!  at the scan edges are read in. Edges are determined in the scaninfo file. 
!  This only applies for TMI even though it occurs for SSMI and other satellite obs.
!  Note, there are not known qulaity issues in these TMI observations.
  integer(i_kind),parameter:: ifail_scanedge_qc=58
!  Reject because sfc wind speed (i.e., 10m wind speed) > 8 m/s
...
subroutine qc_ssmi(nchanl,nsig,ich,sfchgt,luse,sea,mixed, &
     ...
     tmi,gmi,sfc_speed,frac_sea,varinv,errf,aivals,id_qc )
...
!     2014-06-02  j.jin/MJ Kim    - add ifail_scanedge_qc. applies only to TMI
!     2014-06-03  j.jin    - apply this to tmi (images).
!     2014-09-03  j.jin    - apply this for gmi, GMI 1CR, obstype=gmi.
!     2014-10-10  Akella & Jin    - add sfc_speed (wind speed) so as to QC obs at (high) wind speed locations
!     2014-11-18  Jin    & Akella - add qc based on surface height. Over Sea, surface height is strictly = 0.
!
! input argument list:
!     nchanl  - number of channels per obs
!     ich     - channel number
!     sfchgt  - surface height 
...
! Declare passed variables
  ...
  logical                          ,intent(in   ) :: tmi,gmi

  real(r_kind)                     ,intent(in   ) :: sfchgt,tpwc,clw,sgagl,tzbgr,sfc_speed,frac_sea
...
! Set cloud qc criteria  (kg/m2) :  reject when clw>clwcutofx
  if(ssmi) then
     ...
     clwcutofx(1:nchanl) =  &
          ...
          0.10_r_kind, 0.100_r_kind, 0.100_r_kind,                                        &
          0.10_r_kind, 0.10_r_kind, 0.10_r_kind, 0.10_r_kind, 0.10_r_kind/)
...
!    --- amsre separate channel treatment depend on FOV
  ...
  if(gmi) demisf_mi(1:nchanl) = 0.015_r_kind !copied the value from qc_mhs. Jin
...
!          clw QC using ch-dependent threshold (clwch)
     ...
     if(tmi .or. gmi) then
        if(abs(sgagl) < 20.0_r_kind) then
           ...
           if ( abs(sfchgt) > 0.01_r_kind ) then
              do i=1,nchanl
           ...
           if (frac_sea < 0.999_r_kind .or. tzbgr < 275.0_r_kind) then
              do i=1,nchanl
                 if( id_qc(i)== igood_qc ) then
                    varinv(i)=zero
                    id_qc(i)=ifail_emiss_qc
