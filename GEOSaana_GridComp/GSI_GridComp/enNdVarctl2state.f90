subroutine enNdVarctl2state(xhat,mval,filename)
...
use hybrid_ensemble_parameters, only: uv_hyb_ens,dual_res,ntlevs_ens,q_hyb_ens,n_ens
use hybrid_ensemble_isotropic, only: enNDvar_forward_model,enNDvar_forward_model_dual_res
use hybrid_ensemble_isotropic, only: bkgcov_a_en_new_factorization
...
character(len=max_varname_length) :: fname
integer(i_kind) :: jj,nn,ic,id,istatus,nclouds

integer(i_kind), parameter :: ncvars = 8
integer(i_kind) :: icps(ncvars)
type(gsi_bundle):: wbundle_c ! work bundle
type(gsi_bundle),allocatable:: aens(:)  ! ens work bundle
...
! Since each internal vector of xhat has the same structure, pointers are
! the same independent of the subwindow jj
call gsi_bundlegetpointer (mval,mysvars,isps,istatus)
...
! Initialize ensemble contribution to zero
  ...
  do jj=1,ntlevs_ens
     call allocate_state(eval(jj))
  enddo

! Rescale ensemble control variable (need since alpha is precond/d)
! NOTE: might want to accommodate influence of static B, in which 
!       case the lines below should be replaced with something like
!       allocate(tmpcv)
!       call berror_a_en(xhat,tmpcv)
!       aens=tmpcv%a_en
!       deallocate(tmpcv)
  allocate(aens(n_ens))
  do nn=1,n_ens
     call gsi_bundledup(xhat%aens(1,nn),aens(nn),'ens-work',istatus)
  enddo
  call bkgcov_a_en_new_factorization(aens)

! Looping over time internally is more convenient to use
! available routines to write out time-dependent eval
do nn=1,n_ens
...
!  For 4densvar, this is the "3D/Time-invariant contribution from static B"
   ...
   if(dual_res) then
      call enNDvar_forward_model_dual_res(wbundle_c,aens(nn),nn,jj)
   else
      call enNDvar_forward_model(wbundle_c,aens(nn),nn,jj)
   end if
...
! Write out updated ensemble perturbation at this time
  write(fname,'(2a,i3.3)') trim(filename), '_', nn
  call view_st (eval,trim(fname))

end do  ! n_ens

do nn=1,n_ens
  call gsi_bundledestroy(aens(nn),istatus)
enddo
deallocate(aens)
...
! Clear up
  do jj=ntlevs_ens,1,-1
     call deallocate_state(eval(jj))
  enddo
  deallocate(eval)

! Calculate/Write out spread of updated ensemble if requested
  if(write_ens_sprd) then
    do jj=1,ntlevs_ens
       call ens_spread_dualres(jj,prefix='upd')
    enddo
  endif
...
return 
end subroutine enNdVarctl2state
