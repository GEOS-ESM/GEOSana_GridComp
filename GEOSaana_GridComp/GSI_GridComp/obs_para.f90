...
!   2025-10-06  eyang   - remove pblkh and add pblsld, pblgld, ,pblrd
!
!   input argument list:
...
! same as grd_anl/grd_ens, but with communication set up for a single 3d grid
  ...
  use hybrid_ensemble_parameters, only: inf_ensprd_pblsld! if true, inflate ens sprd near pblsld
  use hybrid_ensemble_parameters, only: inf_ensprd_pblgld! if true, inflate ens sprd near pblgld
  use hybrid_ensemble_parameters, only: inf_ensprd_pblrd ! if true, inflate ens sprd near pblrd
  ...
  obs_loop: do n=1,ndata
     ...
     if(lat>=ibs(mm1).and.lat<=ibn(mm1))then
        ...
        if((lon >= ibw(mm1).and. lon <=ibe(mm1))  .or.  &
           ...
           !   pblri and kx=120, cdata_all(17,n)=kx
           if (InfEnsprdPBLH) then
              ...
              if ( ( (trim(obstype)=='pblri' .and. obs_data(17,n)==120 .and. inf_ensprd_pblri) .or. &
                     ...
                     (trim(obstype)=='pblsld' .and. inf_ensprd_pblsld) .or. &
                     (trim(obstype)=='pblgld' .and. inf_ensprd_pblgld) .or. &
                     (trim(obstype)=='pblrd' .and. inf_ensprd_pblrd) ) &
...
!                 print*, 'disobs ens L377: global ens grid degree lat,lon,mm1=', rlats_ens_local(int(dlat_ens))*rad2deg,rlons_ens_local(int(dlon_ens))*rad2deg,mm1
                 ...
                 if (obs_data(5,n) < 0 ) then
                    ...
                    else if (trim(obstype)=='pblrf' .or. trim(obstype)=='pblsld' .or. trim(obstype)=='pblgld' .or. &
                             trim(obstype)=='pblrd' ) then
                 ...
                 !  2b_1) find model level near obs pblh (model geopotential height (geopiges) is interpolated to obs location/time)
                 ...
                 !    - - - - - - - -
                 !    ---------------  1
                 !    if obs is between interface 3 and 4, then we will assign coefficient for
                 !    mid-layer model level 1, 2, 3, 4, and 5 (interface 1-6)
                 !    ges_coef_inf(lat2,lon2,nsig,nfldsig)
                 !    if iz is lowest or izp is highest model, then consider 2 levels
                 !    ---------------------------
                 !    3a_1) for five adjacent vertical model levels
                 !    ---------------------------
                 ...
                 !    ---------------------------
                 !    3a_2) for five adjacent vertical model levels (Gaussian dist.)
                 !    ---------------------------
                 ...
                 !-----------------------------------------------------------------
                 ! Please split the cases for near-surface or near-modeltop later.
                 !-----------------------------------------------------------------

                    ! t
                    ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+2, it:itp) = & 
                    max(ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+2, it:itp), inf_coef2_t)

                    ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+1, it:itp) = & 
                    max(ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+1, it:itp), inf_coef1_t)

                    ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz, it:itp) = &
                    max(ges_coef_inf_ens_grid_global_t(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz, it:itp), inf_coef0_t) 
                    ...
                    max(ges_coef_inf_ens_grid_global_q(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+2, it:itp), inf_coef2_q)

                    ges_coef_inf_ens_grid_global_q(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+1, it:itp) = &
                    max(ges_coef_inf_ens_grid_global_q(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz+1, it:itp), inf_coef1_q)

                    ges_coef_inf_ens_grid_global_q(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz, it:itp) = &
                    max(ges_coef_inf_ens_grid_global_q(ix_ens_global:ixp_ens_global, iy_ens_global:iyp_ens_global, iz, it:itp), inf_coef0_q)
