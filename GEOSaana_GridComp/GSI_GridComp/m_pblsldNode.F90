...
!                .      .    .                                       .
! subprogram:	 module m_pblsldNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblsldNode (surface pblsld)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblsldNode
     !type(pblsld_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblsld residual
     real(r_kind)    :: err2   =0._r_kind       !  pblsld error squared
  ...
  public:: pblsldNode_typecast
  public:: pblsldNode_nextcast
        interface pblsldNode_typecast; module procedure typecast_ ; end interface
        interface pblsldNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblsldNode_appendto
        interface pblsldNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblsldNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblsldNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblsldNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblsldNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblsldNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblsldNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblsldNode),intent(in):: aNode
