...
!   Write observation to scratch file
     ...
     subroutine read_pblri_raob(nread,ndata,nodata,infile,obstype,lunout,twindin,&
         sis,nobs)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  read_pblri_raob     read obs from msgs in NETCDF files
!
! program history log:
...
!     Declare local parameters
      ...
      integer(i_kind), allocatable, dimension(:) :: loc_time, qc_flag
...
!     Read dimensions depending on observation types
      if (index(trim(infile),'raob') > 0) then
...
!        Set usage variable
         ...
         ! qm=9 (stn_elev>3km) for RAOB
         if(pblriqm == 9 .or. pblriqm == 15 ) usage=150.
...
!     Write observation to scratch file
      ...
      end subroutine read_pblri_raob
