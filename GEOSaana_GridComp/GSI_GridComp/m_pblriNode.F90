...
!                .      .    .                                       .
! subprogram:	 module m_pblriNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblriNode (surface pblri)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblriNode
     !type(pblri_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblri residual
     real(r_kind)    :: err2   =0._r_kind       !  pblri error squared
  ...
  public:: pblriNode_typecast
  public:: pblriNode_nextcast
        interface pblriNode_typecast; module procedure typecast_ ; end interface
        interface pblriNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblriNode_appendto
        interface pblriNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblriNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblriNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblriNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblriNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblriNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblriNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblriNode),intent(in):: aNode
