module pblgld_setup
  ...
  private
  public:: setup
        interface setup; module procedure setuppblgld; end interface

contains
subroutine setuppblgld(obsLL,odiagLL,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    setuppblgld    compute rhs for conventional surface pblgld
...
!$$$
  ...
  use m_pblgldNode, only: pblgldNode_appendto
...
! Declare local variables
  ...
  real(r_kind) pblgldges,dlat,dlon,ddiff,dtime,error
  ...
  real(r_kind) cg_pblgld,wgross,wnotgross,wgt,arg,exp_arg,rat_err2
  ...
  integer(i_kind) ier,ilon,ilat,ipblgld,id,itime,ikx,imaxerr,ilocaltime,iqc
...
!  character(15),allocatable,dimension(:):: cdiagbuf
  ...
  type(pblgldNode),pointer:: my_head
  type(obs_diag),pointer:: my_diag
  type(obs_diags),pointer:: my_diagLL
  character(len=*),parameter:: myname='setuppblgld'

  real(r_kind),allocatable,dimension(:,:,:) :: ges_ps
  real(r_kind),allocatable,dimension(:,:,:) :: ges_z
  real(r_kind),allocatable,dimension(:,:,:) :: ges_pblgld

  equivalence(rstation_id,station_id)

  type(obsLList),pointer,dimension(:):: pblgldhead
  pblgldhead => obsLL(:)
...
!  index information for data array (see reading routine)
  ...
  ipblgld=5     ! index of pblgld observation
  id=6        ! index of station id
  itime=7     ! index of observation time in data array
  ikxx=8      ! index of ob type
  !imaxerr=9   ! index of pblgld max error
  ilocaltime=9   ! index of pblgld observation localtime
...
! Check for missing data  !need obs value and error
  do i=1,nobs
    if (abs(data(ipblgld,i)-bmiss) .lt. 10.0_r_kind .or. data(ipblgld,i)<zero)  then
       muse(i)=.false.
       data(ipblgld,i)=rmiss_single   ! for diag output ! -999.0
    end if
  end do
...
! Interpolate to get pblgld at obs location/time
     call tintrp2a11(ges_pblgld,pblgldges,dlat,dlon,dtime,hrdifsig,&
        mype,nfldsig)
...
! Adjust observation error
     ...
     ddiff=data(ipblgld,i)-pblgldges
...
!    Gross check using innovation normalized by error
     if (abs(data(ipblgld,i)-rmiss_single) >= tiny_r_kind ) then
        obserror = one/max(ratio_errors*error,tiny_r_kind)
        obserrlm = max(cermin(ikx),min(cermax(ikx),obserror)) ! ermax for pblgld=1.5
        residual = abs(ddiff)
...
!    Compute penalty terms (linear & nonlinear qc).
     ...
     if(luse(i))then
        ...
        if (vqc .and. cvar_pg(ikx) > tiny_r_kind .and. error > tiny_r_kind) then
           ...
           cg_pblgld=cvar_b(ikx)
           wgross = cg_term*cvar_pg(ikx)/(cg_pblgld*wnotgross)
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        allocate(my_head)
        call pblgldNode_appendto(my_head,pblgldhead(ibin))
...
!        write(station_id,*) int(data(id,i)) by E.-G. Yang commented out
        print*, 'CHECK: setuppblgld dataid,rstation_id,station_id=',data(id,i),rstation_id,station_id
...
! Write information to diagnostic file
  if(conv_diagsave) then
     ...
     if(binary_diag .and. ii>0)then
        write(7)'pblgld',nchar,nreal,ii,mype,ioff0
...
! Check to see if required guess fields are available
  ...
  call gsi_metguess_get ('var::pblgld' , ivar, istatus )
  proceed=proceed.and.ivar>0
...
!    get pblgld ...
     ...
     !print*, "yeg_setuppblgld_init_vars L491: rank2=",rank2
     if (istatus==0) then
         if(allocated(ges_pblgld))then
            write(6,*) trim(myname), ': ', trim(varname), ' already incorrectly alloc '
            call stop2(999)
         endif
         allocate(ges_pblgld(size(rank2,1),size(rank2,2),nfldsig))
         ges_pblgld=0.
         do j=1,size(rank2,2) ! lon
            do i=1,size(rank2,1) ! lat
               ges_pblgld(i,j,1)=rank2(i,j)
            end do
         end do
         do ifld=2,nfldsig
...
900  format('conv_pblgld_',i2.2,'.nc4')
...
!       rdiagbuf(6,ii)  = rmiss_single       ! observation pressure (hPa)
        ...
        rdiagbuf(17,ii) = data(ipblgld,i)      ! PBLH observation (K)
        rdiagbuf(18,ii) = ddiff              ! obs-ges used in analysis (K)
        rdiagbuf(19,ii) = data(ipblgld,i)-pblgldges! obs-ges w/o bias correction (K) (future slot)
  ...
  type(obs_diag),pointer,intent(in):: odiag
! Observation class
  character(7),parameter     :: obsclass = '   pblgld'
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           call nc_diag_metadata("Observation",                   data(ipblgld,i)     )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff             )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", data(ipblgld,i)-pblgldges )
  ...
  subroutine final_vars_
    ...
    if(allocated(ges_pblgld)) deallocate(ges_pblgld)
  end subroutine final_vars_

end subroutine setuppblgld
end module pblgld_setup
