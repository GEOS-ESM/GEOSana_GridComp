...
!                .      .    .                                       .
! subprogram:	 module m_pblrfNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pblrfNode (surface pblrf)
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: pblrfNode
     !type(pblrf_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()
     real(r_kind)    :: res    =0._r_kind       !  pblrf residual
     real(r_kind)    :: err2   =0._r_kind       !  pblrf error squared
  ...
  public:: pblrfNode_typecast
  public:: pblrfNode_nextcast
        interface pblrfNode_typecast; module procedure typecast_ ; end interface
        interface pblrfNode_nextcast; module procedure nextcast_ ; end interface

  public:: pblrfNode_appendto
        interface pblrfNode_appendto; module procedure appendto_ ; end interface

  character(len=*),parameter:: MYNAME="m_pblrfNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(pblrfNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pblrfNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pblrfNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pblrfNode),pointer,intent(in):: aNode
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(pblrfNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(pblrfNode),intent(in):: aNode
