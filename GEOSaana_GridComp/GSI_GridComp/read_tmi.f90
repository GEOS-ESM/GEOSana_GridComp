...
!   2015-05-07  j.jin   - use fitted TMI channle 5 obs data which are derived from co-located TMI and
!                         SSMI observation relation for CLW retrieval.
!   2016-03-22  j.jin   - Set a range (0-360 degree) for satellite and Sun azimuth angles.
...
!  integer(i_kind), parameter      :: nchanl2=7 
!  real(r_kind),dimension(nchanl2) :: tb_tmi2ssmi
...
!             This is not correct for the year 2000.  J.Jin, Oct 15, 2019
              doy = doy + 1
           end if
...
!J.Jin. Not to conduct this cloud check.  08/13/2019
!           if(.not. regional .and. isflg==0 .and. &
!              tmi) then
!             ! retrieve tpwc and clw
!             tb_tmi2ssmi = tbob(3:9)        ! TMI cha 1&2 (10 GHz) are not available in SSMI.
!             !Linear y=a+bx, x=tmi, a, b=     -1.70009      1.03559
!             tb_tmi2ssmi(3) = -1.70009_r_kind + 1.03559_r_kind * tbob(5)
!             call retrieval_mi(tb_tmi2ssmi,nchanl2,no85GHz, &
...
!=========================================================================================================
  if( use_edges .and. (radedge_min > 1 .or. radedge_max < ang_nn).and. mype_sub==mype_root )then
    ...
    !       at the scan edges in the QC process (qc_ssmi, ifail_scanedge_qc=51).
