...
!   2014-04-03  j.jin  - Add process for gmi_gpm
!   2014-09-03  j.jin  - Add indices in data_s for TMI and GMI data, after talking with S. Akella. 
!
...
! ...all "additional absorber" variables
  ...
  integer(i_kind) :: n_channels,i_channel,j_channel
...
! When CW is available in MetGuess, defined Cloudy Radiance for MW only
 lcw4crtm=.false.
 if(trim(obstype)=='amsua' .or. trim(obstype)=='tmi' ) then
...
! Set up index for input satellite data array
 ...
 if ( obstype == 'avhrr_navy' .or. obstype == 'avhrr' ) then         ! when an independent SST analysis is read in
 ...
 else if ( obstype == 'tmi' ) then   ! when an independent SST analysis is read in
   itref     = 35 ! index of foundation temperature: Tr
   idtw      = 36 ! index of diurnal warming: d(Tw) at depth zob
   idtc      = 37 ! index of sub-layer cooling: d(Tc) at depth zob
   itz_tr    = 38 ! index of d(Tz)/d(Tr)
   ...
   ! when an independent SST analysis is read in
   itref     = 40 ! index of foundation temperature: Tr
   idtw      = 41 ! index of diurnal warming: d(Tw) at depth zob
   idtc      = 42 ! index of sub-layer cooling: d(Tc) at depth zob
   itz_tr    = 43 ! index of d(Tz)/d(Tr)
 endif
...
! and those defined and processed (in a subset) by CRTM channelinfo structure.   Return to calling
! routine if there is a mismatch.

 n_channels = crtm_channelinfo_n_channels(channelinfo(sensorindex))
...
! Allocate structures for radiative transfer
 ...
 allocate(&
    rtsolution  (n_channels,1),&
    rtsolution_k(n_channels,1),&
    atmosphere_k(n_channels,1),&
    surface_k   (n_channels,1))
...
!_RTod-NOTE: the following will work in single precision but issue lots of msg and remove more obs than needed
    ...
    call crtm_surface_create(surface(1),n_channels)
!_RTod-NOTE endif
...
! Check for consistency with information in crtm for number of channels
 ...
 if(nchanl /= n_channels) write(6,*)'***ERROR** nchanl,n_channels ', &
    nchanl,n_channels

! Load surface sensor data structure

 surface(1)%sensordata%n_channels = n_channels

!! REL-1.2 CRTM
!!  surface(1)%sensordata%select_wmo_sensor_id  = channelinfo(1)%wmo_sensor_id
!! RB-1.1.rev1855 CRTM

 ! j.jin, 20140403: The two gmi_gpm ID numbers are still preliminary from WMO 
 !                  and change in the future. Should not need to set these here
 !                  once CRTM has values accordingly set.
 if(trim(isis)=='gmi_gpm') then
    channelinfo(sensorindex)%WMO_sensor_id = 519
    channelinfo(sensorindex)%WMO_Satellite_id = 288
 endif
 ...
 j_channel = 0
 do i_channel = 1, channelinfo(sensorindex)%n_channels
   if( channelinfo(sensorindex)%Process_Channel(i_channel) ) then
     j_channel = j_channel + 1
     surface(1)%sensordata%sensor_channel(j_channel) = channelinfo(sensorindex)%sensor_channel(i_channel)
   endif
 enddo
...
!          GFS and NNM as of September 2005
     ...
     if ((ABS(uu5)>zero .or. ABS(vv5)>zero) .and. lwind) then
       ...
       surface(1)%wind_direction       = rad2deg*atan2(uu5,vv5) + 180.0_r_kind
