...
! real(r_kind)                   :: tdiff, sstime, crit1, dist1, timedif
  real(r_kind)                   :: crit1, dist1, timedif
  integer(i_kind)                :: i, kk, itx, itt
  logical                        :: iuse
...
! Fill with negative so we don't accidentally misuse
  ...
  if (isdoas) then
     allocate(szas1(nsound), cldfrcs1(nsound), qflags1(nsound),                &
              albds1(nsound), qav1(nsound), rows(nsound), tropp(nsound),       &
              vcds(nsound), amfs(nsound))
     allocate(scwtpress(navg), siga(navg), sigb(navg))
  end if
  id_qav = -1

! Allocate output array
  ntgasdat = nreal + nchanl
  itxmax = min(itxmax, maxobs)
  allocate(tgasout(ntgasdat,itxmax))
! NEEDED FOR THINNING (UGH)
  tgasout = zero
...
!    Toss out any obs with suspicious locations
     ...
     ! Thin (should do nothing if ithin == 0)
     iuse = .true.
     if (l4dvar .or. l4densvar) then
        timedif = zero
     else
        timedif = abs(nmind - gstime)/10.0_r_kind
     end if
...
!    Write record to output file
     tgasout(1,itx) = n
     tgasout(2,itx) = grdtime
     tgasout(3,itx) = grdlon
     tgasout(4,itx) = grdlat
     tgasout(5,itx) = deglon
     tgasout(6,itx) = deglat

     iout = 6
     do k = 1,nchanl
        iout = iout + 1
        tgasout(iout,itx) = pchanls(k,n)             ! channel pressure
     end do
     do k = 1,nchanl
        iout = iout + 1
        tgasout(iout,itx) = isbads(k,n)              ! qc flag
     end do
     do k = 1,nchanl
        iout = iout + 1
        tgasout(iout,itx) = uncerts(k,n)             ! channel uncertainty
     end do 

     if (useak) then
        do k = 1,nedge
           iout = iout + 1
           tgasout(iout,itx) = peavgs(k,n)           ! averaging kernel edge pressures
        end do
        do k = 1,navg
           do j = 1,nchanl
              iout = iout + 1
              tgasout(iout,itx) = avgkers(j,k,n)     ! averaging kernel
           end do
        end do
        do k = 1,navg
           iout = iout + 1
           tgasout(iout,itx) = priorpros(k,n)        ! a priori profile
        end do 
        do j = 1,nchanl
           iout = iout + 1
           tgasout(iout,itx) = priorobses(j,n)       ! a priori obs data
        end do
     end if

     do k = 1,nchanl
        iout = iout + 1
        tgasout(iout,itx) = obses(k,n)               ! observation
     end do
