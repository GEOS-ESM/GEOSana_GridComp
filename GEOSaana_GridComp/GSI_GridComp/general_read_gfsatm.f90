subroutine general_read_gfsatm(grd,sp_a,sp_b,filename,mype,uvflag,en_read,iret_read)
...
!   2014-11-30  todling    - update interface: pass bundle; internal code should be generalized
!
!   input argument list:
...
!     en_read   - guess fields in bundle form
...
!   Declare local variables
    ...
    integer(i_kind) i,j,k,icount,ier,istatus
    ...
    real(r_kind),        dimension(grd%lat2,grd%lon2) :: g_z
...
!   Get pointer to relevant variables (this should be made flexible and general)
    call gsi_bundlegetpointer(en_read,'ps',g_ps  ,ier);istatus=ier
    call gsi_bundlegetpointer(en_read,'sf',g_div ,ier);istatus=istatus+ier
    call gsi_bundlegetpointer(en_read,'vp',g_vor ,ier);istatus=istatus+ier
    call gsi_bundlegetpointer(en_read,'t' ,g_tv  ,ier);istatus=istatus+ier
    call gsi_bundlegetpointer(en_read,'q' ,g_q   ,ier);istatus=istatus+ier
    call gsi_bundlegetpointer(en_read,'oz',g_oz  ,ier);istatus=istatus+ier
    call gsi_bundlegetpointer(en_read,'cw',g_cwmr,ier);istatus=istatus+ier
    if (istatus/=0) then
       if (mype==0) then
