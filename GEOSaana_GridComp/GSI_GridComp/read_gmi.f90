...
! Declare local variables
  logical        :: assim,outside,iuse, no85GHz, gmi
...
!          reset observation to "bad" value.
           ...
           do jc=1, nchanla    ! only does such check the first 9 channels for GMI 1C-R data
              if(mirad(jc)<tbmin(jc) .or. mirad(jc)>tbmax &
                .or. gmichq(jc) < -0.5_r_kind .or. gmichq(jc) > 1.5_r_kind) then
                 iskip = iskip + 1
              else
                 nread=nread+1
              end if
           enddo
           if(iskip == nchanla)  cycle read_loop 
           tbob = mirad 
...
!                  4 mixed                     
           ...
           if(.not. regional .and. isflg==0) then
             ...
             if(tpwc<0 .or. clw > 5 .or. kraintype /=0_i_kind .or. ierrret > 0) cycle read_loop
           endif
