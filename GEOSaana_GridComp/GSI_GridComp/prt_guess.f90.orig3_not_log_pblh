...
! Declare local variables
  integer(i_kind), parameter :: nvars=16
  ...
  real(r_kind),pointer,dimension(:,:  )::ges_pblkh_it=>NULL()
...
!*******************************************************************************
  ...
  call gsi_bundlegetpointer (gsi_metguess_bundle(ntsig),'pblkh',ges_pblkh_it,istatus)
...
! get pointer to cloud water condensate
  ...
  zloc(16)         = sum   (ges_pblkh_it(2:lat1+1,2:lon1+1))
  ...
  zloc(nvars+16)   = minval(ges_pblkh_it(2:lat1+1,2:lon1+1))
  ...
  zloc(2*nvars+16) = maxval(ges_pblkh_it(2:lat1+1,2:lon1+1))
