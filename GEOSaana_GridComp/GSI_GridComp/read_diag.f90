...
!   2020-03-13 sienkiewicz -  nc4 BC_angord now does not include a copy of the BC_Fixed_Scan_Position
!                              - change to put a copy in diag_status%all_data_chan(ir,cch)%bifix(1)
!
! contains
...
!  allocate(  all_data_chan(nrecord, nchan))
  ...
  do ir=1,nrecord
    ...
    do ic=1,header_fix%nchan
      ...
      if (header_fix%angord > 0) then
         allocate(diag_status%all_data_chan(ir,cch)%bifix(nangord+1))
      else
      ...
      if (header_fix%angord > 0) then
         diag_status%all_data_chan(ir,cch)%bifix(1:nangord) = BC_angord(1:nangord,cdatum)
         diag_status%all_data_chan(ir,cch)%bifix(nangord+1) = BC_Fixed_Scan_Position(cdatum) 
      else
         diag_status%all_data_chan(ir,cch)%bifix(1) = BC_Fixed_Scan_Position(cdatum)
      endif
!!$      diag_status%all_data_chan(ir,cch)%bifix(1) = BC_Fixed_Scan_Position(cdatum)
!!$      if (header_fix%angord > 0) then
!!$         diag_status%all_data_chan(ir,cch)%bifix(2:nangord+1) = BC_angord(1:nangord,cdatum)
!!$      endif
