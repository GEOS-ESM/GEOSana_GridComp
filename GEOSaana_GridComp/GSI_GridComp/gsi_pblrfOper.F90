module gsi_pblrfOper
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:	 module gsi_pblrfOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for pblrfNode type
!
! program history log:
...
! module interface:
  ...
  implicit none
  public:: pblrfOper      ! data stracture

  type,extends(obOper):: pblrfOper
  contains
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_pblrfOper'
  type(pblrfNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(pblrfOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    use intpblrfmod, only: intjo => intpblrf
    ...
    implicit none
    class(pblrfOper  ),intent(in   ):: self
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    use stppblrfmod, only: stpjo => stppblrf
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
    ...
    headNode => obsLList_headNode(self%obsLL(ibin))
    print*, "YEGPL_gsi_pblrfOper stpjo L157 before call stpjo"
    call stpjo(headNode,dval,xval,pbcjo(:),sges,nstep)
    print*, "YEGPL_gsi_pblrfOper stpjo L159 after call stpjo"
    headNode => null()
  end subroutine stpjo1_

end module gsi_pblrfOper
